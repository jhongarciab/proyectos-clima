...
\pagestyle{fancy}
\fancyhf{}
\lhead{\small Pereira 2016--2025}
\rhead{\small Mini Proyecto 1}
\rfoot{\thepage}
...
\begin{document}
	...
	\begin{titlepage}
		...
		{\LARGE \textbf{Análisis Climático de Pereira, Risaralda}\\[0.5em]
			...
			y reanálisis NASA POWER (2016–2025)}\\[2cm]
...
un clima cálido y húmedo con escasa variación térmica a lo largo del año \cite{poveda2004}.
Por esta combinación de factores (altitud media, exposición a masas de aire húmedo,
ubicación en una región de alta densidad poblacional y actividad agrícola) Pereira
constituye un caso de estudio representativo del clima de la montaña tropical colombiana
y un punto de referencia útil para evaluar la capacidad de distintas fuentes de datos
de capturar la variabilidad climática local \cite{urrea2019}.

Con ese propósito, el análisis cubre el período 2016--2025 y emplea dos fuentes de
datos de naturaleza muy distinta. La primera son las observaciones directas de tres
estaciones meteorológicas del Instituto de Hidrología, Meteorología y Estudios
Ambientales (IDEAM) ubicadas en el área urbana y periurbana de Pereira: el Aeropuerto
Internacional Matecaña, la estación La Catalina y El Pílamo, ubicadas a 1\,342,
1\,321 y 1\,113~metros sobre el nivel del mar, respectivamente. Los registros
originales, con resolución de cinco minutos, fueron descargados mediante la API de
Datos Abiertos del gobierno nacional \cite{ideam_temperatura, ideam_humedad,
	ideam_precipitacion}, la cual recoge datos hidrometeorológicos crudos mediante una
red de estaciones del IDEAM, y fueron agregados a escala diaria promediando temperatura
y humedad relativa, y acumulando la precipitación. El promedio entre estaciones con
dato disponible para cada fecha constituye la serie observacional representativa de
Pereira utilizada en el análisis.

La segunda fuente es el proyecto NASA POWER \cite{nasa_power}, que provee datos
diarios derivados del modelo de reanálisis MERRA-2 \cite{gelaro2017, stackhouse2018}
para cualquier punto del globo. Se utilizó la celda de rejilla centrada en las
coordenadas 4.8143°N, 75.7446°W, como se puede observar en la Figura
\ref{fig:nasa:power}, con resolución espacial de 0.5°~$\times$~0.625° en latitud y
longitud. La elevación promedio que MERRA-2 asigna a esta celda es de 2\,138~metros
sobre el nivel del mar, valor sensiblemente superior a la altitud real de Pereira
porque el modelo integra la topografía de una región extensa que incluye zonas de
mayor elevación de la cordillera.
...
valores centinela, errores de sensor y registros fuera de rango físicamente plausible.
A partir de estas series limpias y continuas se analiza la climatología base de
Pereira, su variabilidad interanual, la señal de eventos ENSO \cite{poveda2011} y
las periodicidades dominantes mediante análisis espectral.
...
para humedad relativa. Los Q-Q plots, los valores del estadístico KS y
los AIC por variable y distribución candidata están disponibles en el repositorio
del proyecto. El volumen de datos imputados fue de 483 registros en
...
El flujo completo de procesamiento, con el fin de garantizar trazabilidad y reproducibilidad desde la ingesta de los archivos crudos hasta
...
promedio de la celda MERRA-2, que integra una región que alcanza los 2\,138~m
\cite{gelaro2017}. Más allá de ese offset, ambas fuentes reproducen
la misma variabilidad temporal: los mismos episodios de calentamiento, los mismos
períodos fríos, la misma ausencia de una estacionalidad térmica marcada, característica
definitoria del clima ecuatorial andino.
...
\begin{figure}[ht]
	...
	\caption{Series de tiempo diarias de temperatura (°C), precipitación (mm/día) y
		humedad relativa (\%) para Pereira 2016--2025.}
...
\begin{figure}[ht]
	...
	\caption{Series de tiempo mensuales de temperatura (°C), precipitación (mm/mes)
		y humedad relativa (\%) para Pereira 2016--2025.}
...
variabilidad interanual, lo que sugiere que en Pereira el concepto de estación
térmica carece de significado práctico \cite{poveda2004}. La precipitación mensual es, con diferencia, la variable más ruidosa y la que
acumula mayor variabilidad tanto estacional como interanual. Ambas fuentes dibujan
el mismo esqueleto bimodal (dos crestas separadas por dos valles) \cite{urrea2019}
pero con amplitudes distintas: el IDEAM registra eventos mensuales que superan los
400~mm mientras que NASA POWER raramente excede los 350~mm. 
...
(junio--agosto) y Lluvioso~2 (septiembre--noviembre).
La temperatura confirma su notable estabilidad estacional: la diferencia entre la
estación más cálida y la más fresca no supera 0.5~°C en IDEAM, con Seco~1 siendo
marginalmente el período más cálido (22.5~°C) y Lluvioso~2 el más fresco (22.0~°C).
NASA POWER replica este patrón a su nivel altitudinal, oscilando entre 16.2 y
16.7~°C con la misma estructura relativa entre estaciones. La humedad relativa sí
exhibe una respuesta estacional reconocible, aunque moderada. IDEAM muestra su valor
más alto en Lluvioso~1 (80\%) y más bajo en Seco~1 (76\%), con una diferencia entre
estaciones de apenas cuatro puntos porcentuales. NASA POWER presenta valores
consistentemente más altos (85--87\%) y con aún menor variación estacional (2\%),
reflejo de su menor sensibilidad a los extremos locales. La concordancia entre fuentes
en la dirección del cambio estacional (ambas identifican Lluvioso~1 como el período
más húmedo y Seco~1 como el más seco) es un resultado positivo que valida la
coherencia física de ambos conjuntos de datos \cite{poveda2004}.
...
característica de doble joroba \cite{urrea2019, poveda2005}: el primer pico se alcanza en marzo (200~mm en IDEAM), se sostiene en abril y mayo y cae hacia el mínimo relativo de julio (160~mm); el segundo pico ocurre en octubre y noviembre, con noviembre como el mes más lluvioso del año: 320~mm en IDEAM y 200~mm en NASA POWER. Los meses más secos son enero y febrero, con menos de 110~mm en ambas
fuentes. La mayor intensidad del segundo período lluvioso respecto al primero está
asociada a la posición más austral de la ZCIT en su segundo paso anual, que coincide
con una mayor disponibilidad de humedad amazónica y un reforzamiento de la convección
en los flancos andinos \cite{hoyos2018, poveda2011}. Resulta notable que Seco~2
(junio--agosto) no sea realmente seco en términos absolutos: con valores entre 160
y 180~mm/mes, supera a muchos meses que en otras regiones se considerarían lluviosos.
Esta característica es propia del clima ecuatorial húmedo, donde incluso los períodos
de menor precipitación mantienen una pluviosidad considerable \cite{poveda2004}.

La asimetría entre los dos picos lluviosos, con Lluvioso~2 notablemente más intenso
que Lluvioso~1 en los datos del IDEAM pero no en NASA POWER, se explica por la
dinámica del segundo paso de la ZCIT sobre Colombia \cite{urrea2019}. Este paso,
que ocurre entre septiembre y noviembre en su recorrido de regreso hacia el ecuador
austral, coincide con una mayor disponibilidad de energía y humedad: la Amazonía ha
acumulado calor durante el verano boreal y el Pacífico tropical oriental mantiene
temperaturas superficiales elevadas, intensificando la convección sobre los flancos
andinos \cite{hoyos2018, poveda2011}. Este reforzamiento es un fenómeno de mesoescala
estrechamente vinculado a la topografía local de los Andes y a la interacción entre
sus flancos \cite{poveda2004}, que una celda de rejilla de 0.5°~$\times$~0.625° no
tiene resolución suficiente para capturar. NASA POWER promedia
...
extremos puntuales que sí capturan las estaciones de superficie.
Pese a esa diferencia de amplitud, ambas fuentes comparten el signo de la anomalía
la mayor parte del tiempo, lo que confirma que responden a los mismos forzamientos
atmosféricos de gran escala \cite{poveda2011}.
...
\begin{figure}[ht]
	...
	\caption{Anomalías diarias de temperatura (°C), precipitación (mm/día) y humedad
		relativa (\%) respecto a la climatología diaria del período 2016--2025.}
...
positiva.Los valores negativos presentan menor amplitud en ambas fuentes y muestran un comportamiento más similar entre ellas que los positivos: la precipitación está acotada inferiormente por cero, de modo que las
...
POWER \cite{ideam2019nino}. Este episodio, que se extiende de manera casi
ininterrumpida durante aproximadamente un año, apunta a un déficit hídrico local
de origen sinóptico que el reanálisis, al promediar condiciones regionales, no es
capaz de reproducir \cite{hoyos2018}.
...
fondo \cite{raghuraman2024}. En el extremo opuesto, 2021 y 2022 presentan anomalías
negativas consistentes en temperatura en ambas fuentes, coherentes con el enfriamiento
asociado a La Niña que dominó el Pacífico tropical durante ese bienio \cite{poveda2011}.
...
a escala regional \cite{gelaro2017}, mientras que las estaciones de superficie del
IDEAM reflejan condiciones más locales que pueden diferir del promedio regional
\cite{urrea2019}. El déficit de humedad de 2019--2020 también es perfectamente
legible a escala mensual en IDEAM, con anomalías que en los meses centrales del
episodio superan los $-$14~\%, y ausente en NASA POWER \cite{ideam2019nino},
reforzando la interpretación de un fenómeno de escala local \cite{hoyos2018}.
...
\begin{figure}[ht]
	...
	\caption{Anomalías anuales de temperatura (°C), precipitación (mm/año) y
		...
		años con anomalía positiva. Barras claras: años con anomalía negativa.}
	\label{fig:anomalias_anuales}
\end{figure}
La Figura~\ref{fig:anomalias_anuales} ofrece la lectura interanual más clara del período al integrar las anomalías por año completo. El panel de temperatura ofrece la lectura más limpia:
...
clima del Eje Cafetero: los años de El Niño tienden a ser más cálidos y secos \cite{ideam2019nino, raghuraman2024}, mientras que los de La Niña son más frescos y húmedos \cite{poveda2011}. La concordancia entre IDEAM y NASA
POWER en el signo y la secuencia de las anomalías térmicas anuales es el resultado
de mayor consistencia entre fuentes en todo el análisis, lo que valida que ambas
capturan correctamente la variabilidad interanual de gran escala
aun cuando difieren en amplitud y en la resolución de fenómenos locales.
...
hacia el frío que hacia el calor \cite{poveda2004}. NASA POWER muestra una
distribución más estrecha y con menor asimetría, centrada en 16.4~°C, confirmando
el desplazamiento altitudinal ya discutido \cite{gelaro2017}. La menor dispersión
del reanálisis respecto a las observaciones de superficie es coherente con su
naturaleza de promedio espacial sobre una celda extensa.
...
1.4\%. Esta diferencia de 28 puntos porcentuales no es un error sino una
consecuencia directa de la escala espacial \cite{urrea2019}: una estación puntual
puede registrar cero milímetros en un día en que llueve a pocos kilómetros de
distancia, mientras que la celda de reanálisis, al promediar sobre cientos de
kilómetros cuadrados, casi siempre acumula alguna precipitación en algún punto de
su área. En los días con lluvia, ambas distribuciones presentan
la forma exponencial decreciente característica de la precipitación convectiva
tropical \cite{poveda2005}: eventos frecuentes de baja intensidad y eventos raros
de alta intensidad. La diferencia fundamental está en los extremos: el IDEAM registra
eventos diarios que superan los 100~mm, mientras que el máximo de NASA POWER en días
lluviosos no alcanza los 70~mm, reflejo nuevamente del suavizado espacial del
reanálisis.
...
altitudinal directa como en temperatura; la humedad relativa no sigue un gradiente
altitudinal simple, sino que refleja que el reanálisis representa condiciones medias
regionales donde la humedad permanece consistentemente elevada \cite{hoyos2018},
...
estacionales \cite{poveda2004}. NASA POWER sí presenta un pico más definido en
torno al período anual, lo que sugiere que el reanálisis introduce cierta
estacionalidad térmica regional que las estaciones locales no registran con la
misma intensidad \cite{gelaro2017}. A períodos más largos que el anual, la potencia
espectral crece monótonamente en ambas fuentes, reflejo de la variabilidad interanual
de baja frecuencia asociada al ciclo ENSO \cite{poveda2011}.
...
del Eje Cafetero \cite{urrea2019}. Este es el resultado espectral central del
análisis: la organización climática de Pereira en materia de lluvias no responde
a un ciclo anual sino a uno semianual, impuesto por el doble paso de la ZCIT
\cite{poveda2005, hoyos2018}. La potencia de IDEAM supera a la de NASA POWER en
el rango de períodos superiores a 1\,000 días, señal de que las observaciones de
superficie capturan con mayor fidelidad la variabilidad interanual de la
precipitación, incluyendo los años extremos identificados en la sección anterior.
...
El zoom sobre la banda 1–400 días de la Figura~\ref{fig:fourier_zoom} revela un detalle que el espectro completo no hacía visible: en temperatura, IDEAM presenta un pico pronunciado en
...
lluvias a través de la nubosidad y la evapotranspiración \cite{poveda2004}. En
precipitación, el pico semianual es el más claro del espectro en IDEAM, confirmando
que el bimodal es la frecuencia dominante de organización pluviométrica \cite{urrea2019}.
En períodos cortos, por debajo de los 30 días, ambas fuentes presentan un espectro
de tipo ruido rojo sin picos discretos identificables: no existe ningún ciclo
semanal, decenal ni mensual que organice la precipitación local, lo que es
característico de los climas convectivos tropicales donde la lluvia es
fundamentalmente estocástica a escala de días \cite{poveda2005}.
...
\cite{ideam2019nino}. NASA POWER presenta un espectro de humedad más plano y con
menor potencia en todas las frecuencias, coherente con su menor sensibilidad a los
extremos locales \cite{gelaro2017}. La coincidencia entre ambas fuentes en la
ubicación del pico semianual, presente en las tres variables, constituye la
validación espectral del patrón bimodal y confirma que este ciclo es una
característica robusta del clima de Pereira, reproducible independientemente de
la fuente de datos utilizada \cite{urrea2019, poveda2005}.
...
\begin{thebibliography}{9}
	...
	\bibitem{poveda2004}
	Poveda, G.
	La hidroclimatología de Colombia: una síntesis desde la escala inter-decadal
	hasta la escala diurna.
	\textit{Revista de la Academia Colombiana de Ciencias Exactas, Físicas y
		Naturales}, 28(107), 201--222, 2004.
	
	\bibitem{urrea2019}
	Urrea, V., Ochoa, A., y Mesa, O.
	Seasonality of Rainfall in Colombia.
	\textit{Water Resources Research}, 55(5), 4149--4162, 2019.
	\url{https://doi.org/10.1029/2018WR023316}
	
	\bibitem{ideam_temperatura}
	Instituto de Hidrología, Meteorología y Estudios Ambientales (IDEAM).
	Temperatura Ambiente del Aire.
	Datos Abiertos Colombia, 2025.
	\url{https://www.datos.gov.co/Ambiente-y-Desarrollo-Sostenible/Temperatura-Ambiente-del-Aire/sbwg-7ju4/about_data}
	
	\bibitem{ideam_humedad}
	Instituto de Hidrología, Meteorología y Estudios Ambientales (IDEAM).
	Humedad del Aire.
	Datos Abiertos Colombia, 2025.
	\url{https://www.datos.gov.co/Ambiente-y-Desarrollo-Sostenible/Humedad-del-Aire/uext-mhny/about_data}
	
	\bibitem{ideam_precipitacion}
	Instituto de Hidrología, Meteorología y Estudios Ambientales (IDEAM).
	Precipitación.
	Datos Abiertos Colombia, 2025.
	...
	NASA Langley Research Center, 2025.
	...
	Version 2 (MERRA-2).
	\textit{Journal of Climate}, 30(14), 5419--5454, 2017.
	\url{https://doi.org/10.1175/JCLI-D-16-0758.1}
	
	\bibitem{stackhouse2018}
	Stackhouse, P. W., Zhang, T., Westberg, D., Barnett, A. J., Bristow, T.,
	Macpherson, B., y Hoell, J. M.
	\textit{POWER Release 8 (with GIS Applications) Methodology (Data Parameters,
		Sources, \& Validation)}, versión de datos 8.0.1.
	NASA Technical Report, 2018.
	\url{https://power.larc.nasa.gov/}
	
	\bibitem{poveda2011}
	Poveda, G., Álvarez, D. M., y Rueda, O. A.
	Hydro-climatic variability over the Andes of Colombia associated with ENSO:
	a review of climatic processes and their impact on one of the Earth's most
	important biodiversity hotspots.
	\textit{Climate Dynamics}, 36(11--12), 2233--2249, 2011.
	\url{https://doi.org/10.1007/s00382-010-0931-y}
	
	\bibitem{poveda2005}
	Poveda, G., Jaramillo, A., Gil, M. M., Quiceno, N., y Mantilla, R. I.
	The Diurnal Cycle of Precipitation in the Tropical Andes of Colombia.
	\textit{Monthly Weather Review}, 133(1), 228--240, 2005.
	\url{https://doi.org/10.1175/MWR-2853.1}
	
	\bibitem{hoyos2018}
	Hoyos, I., Dominguez, F., Cañón-Barriga, J., Martínez, J. A., Nieto, R.,
	Gimeno, L., y Dirmeyer, P. A.
	Moisture origin and transport processes in Colombia, northern South America.
	\textit{Climate Dynamics}, 50(3--4), 971--990, 2018.
	\url{https://doi.org/10.1007/s00382-017-3653-6}
	
	\bibitem{ideam2019nino}
	Instituto de Hidrología, Meteorología y Estudios Ambientales (IDEAM).
	\textit{Boletín informativo sobre el monitoreo del fenómeno El Niño},
	No. 24.
	Ministerio de Ambiente y Desarrollo Sostenible, Bogotá, Colombia, 2019.
	\url{https://www.ideam.gov.co/web/tiempo-y-clima/vigilancia-enso}
	
	\bibitem{raghuraman2024}
	Raghuraman, S. P., Soden, B., Clement, A., Vecchi, G., Menemenlis, S.,
	y Yang, W.
	The 2023 global warming spike was driven by the El Niño--Southern Oscillation.
	\textit{Atmospheric Chemistry and Physics}, 24, 11275--11283, 2024.
	\url{https://doi.org/10.5194/acp-24-11275-2024}
